...
public class JWTUtil {
	...
	private static Key getKeyInstance() {
		...
		byte[] apiKeySecretBytes = DatatypeConverter.parseBase64Binary("Ready");
	...
	public static String createJWT(String id, String subject, long ttlMillis) {
		...
		// 根据信息生成token
		JwtBuilder builder = Jwts.builder().claim("userId", id).setId(id).setSubject(subject).setIssuedAt(now).signWith(signatureAlgorithm,
				secretKey);
